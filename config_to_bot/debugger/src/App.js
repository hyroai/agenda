...
const renderDebugState = ([key, value], i) => (
  <div key={i} marginRight={1}>
    <span>{key}: </span>
    <span style={{color: "red"}}>{JSON.stringify(value).replace("null", "?")}</span>
  </div>
);
...
const textToBotUtterance = ({ botUtterance, state }) => (
  <div style={{flexDirection: "row", justifyContent: "flex-start", gap:20, display: "flex"}}>
...
const App = () => {
  ...
  useEffect(() => {
    ...
    if (readyState === ReadyState.CONNECTING) addEvent({ userUtterance: "reset" });
  }, [readyState, addEvent]);
